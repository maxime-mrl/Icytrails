...
<form action="/users/settings" method="post" class="update-infos">
    <p>You can leave blank all value that should not be updated</p>
        ...
        <input type="text" name="username" id="update-name" placeholder="Your even cooler username" autocomplete="name" data-err="Please enter a valid username" data-check="/^[-a-z0-9/]{4,20}$|^$/i" value="<?= $username ?>">
        ...
        <label for="update-check-pass">Confirm your actual password</label>
        <input type="password" name="pass" id="update-check-pass" placeholder="Your old password" data-err="Password must be at least 6 characters" data-check="/^.{6,}$/" required>
...
<form class="update-infos" action="/users/delete" method="post" onsubmit="return confirm('Are you sure you want to delete your account?');">
    <label for="delete-check-pass">Confirm your actual password</label>
    <input type="password" name="pass" id="delete-check-pass" placeholder="Your password" data-err="Password must be at least 6 characters" data-check="/^.{6,}$/" required>
